...
const Editor = () => {
  ...
  const handleGenerateFromLogs = useCallback(async (parentNode) => {
    if (!parentNode) return;
    ...
    try {
      ...
      const logResponses = result.logNodes || [];

      if (logResponses.length === 0) {
        toast.success('No log suggestions found for this node', { id: 'generate-logs', duration: 3000 });
        return;
      }

      const positions = calculateChildrenPositions(parentNode, logResponses.length, state.tree);
      const logNodes = logResponses.map((response, index) => {
        const text = typeof response === 'string' ? response : (response.text || '');
        const description = typeof response === 'object' ? (response.description || '') : '';
        const node = new MindMapNode(
          `node-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`,
